...
function [code, codelength, efficiency] = my_shannon_fano(probability)
    p = probability(:);
    ...
    if length(subset1_idx) > 1
        [subset1_code, subset1_codelength, subset1_efficiency] = my_shannon_fano(p(subset1_idx));
    ...
    if length(subset2_idx) > 1
        [subset2_code, subset2_codelength, subset2_efficiency] = my_shannon_fano(p(subset2_idx));
